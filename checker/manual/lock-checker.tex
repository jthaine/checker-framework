...
\item[\refqualclass{checker/lock/qual}{GuardedBy}{\small(\emph{exprSet})}]
  ...
  default type qualifier for all locations in unannotated libraries
...
\item[\refqualclass{checker/lock/qual}{GuardSatisfied}{\small(\emph{index})}]
  ...
  \code{@GuardSatisfied} is only allowed on primary annotations of
...
\paragraph{Polymorphic assignments}
...
\item[\code{@GuardSatisfied}]
  ...
  Given a method with a formal parameter with type annotation \<@GuardSatisfied(index)>,
  that parameter can be assigned to within that method's body: \<@GuardSatisfied(index)>
  can only be re-assigned from a right-hand-side of identical \<@GuardSatisfied(index)> type.
  A formal parameter with type annotation \<@GuardSatisfied> without an index cannot be
  assigned to.

  When type checking a call to a method with a receiver, return type and/or formal parameters with types annotated with
  \<@GuardSatisfied(index)>, the Lock Checker ensures that for all \<@GuardSatisfied> annotations
  with the same index, the corresponding primary annotations in the \<@GuardedBy> hierarchy
  ...
  annotated with \<@GuardSatisfied> without an index, then those formal parameter
  types are unrelated with regard to types in the \<@GuardedBy> hierarchy.
...
\paragraph{Primitive types, boxed primitive types and Strings}

Primitive types, boxed primitive types (such as \<java.lang.Integer>), and type \<java.lang.String>
are treated as being annotated with \<@GuardedBy(\{\})>.
...
\subsection{Polymorphism and method formal parameters with unknown guards\label{lock-checker-polymorphism}}
...
arbitrary sets of locks, regardless of any dereferences of the receiver and parameter
that the method body of \code{append} may perform, provided that those locks are held at the method
call site.
